...
if [ -z "$LR" ] || [ -z "$BATCH_SIZE" ] || [ -z "$ITERS" ]; then
  echo "Usage: sbatch fm-ct.qs <lr> <batch_size> <iterations>"
  exit 1
fi

# Define output directory
OUTDIR="fm-ct-run-lr-${LR}-bs-${BATCH_SIZE}-iterations-${ITERS}"
mkdir -p $OUTDIR
...
python fm-ct.py --lr $LR --batch_size $BATCH_SIZE --iterations $ITERS --outdir $OUTDIR
